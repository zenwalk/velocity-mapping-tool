...
xyuvvrNN(:,1) = x(nonNaNind);
xyuvvrNN(:,2) = y(nonNaNind);
xyuvvrNN(:,3) = u(nonNaNind);
xyuvvrNN(:,4) = v(nonNaNind);
xyuvvrNN(:,5) = vr(nonNaNind);
...
for it=1:n
    ...
    si = iData(it)+1;
    ei = iData(it+1);
    %hh=quiver(xyuvvrNNs(si:ei,1),xyuvvrNNs(si:ei,2),xyuvvrNNs(si:ei,3),xyuvvrNNs(si:ei,4),scale*it/n,'Color',c,exag);
    hh=quiver(xyuvvrNNs(si:ei,1),xyuvvrNNs(si:ei,2),xyuvvrNNs(si:ei,3),xyuvvrNNs(si:ei,4),scale*it/n,'Color',[0 0 0],exag);  %Specifies single color (black) for all vectors
end    
...
exag = varargin{end};
alpha = 0.33; % Size of arrow head relative to the length of the vector 0.33
beta = 0.33./exag;  % Width of the base of the arrow head relative to the length 0.33./exag
