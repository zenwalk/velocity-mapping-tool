function VMT_PlotPlanViewQuiversMATv3(zPathName,zFileName,zf,drng,ascale,QuiverSpacing,pvsmwin,pshore,plot_english)
...
%v3 adds the ability to change the plotting units to english
...
% Save the planview data to an *.anv file with spacing and smoothing (for iRiC) 
if savePVdata
    ...
    [file,path] = uiputfile('*.anv','Save *.anv file',zPathName)
    outfile = [path file];
    disp(outfile)
    ofid   = fopen(outfile, 'wt');
    outcount = fprintf(ofid,'%8.2f  %8.2f  %5.2f  %3.3f  %3.3f\n',outmat(goodrows,:)');
    fclose(ofid);
    % header X,Y,Z(zeros),Ve,Vn
end
