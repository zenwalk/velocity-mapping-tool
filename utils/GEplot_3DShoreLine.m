function GEplot_3DShoreline(filename,Lat,Lon,Elev,style,opt11,opt12,opt21,opt22)
%
%Shoreline does not tasselate or extrude 
...
% Argument checking
%
error(nargchk(4, 9, nargin));  %3 arguments required, 8 maximum
n1=length(Lat);
n2=length(Lon);
n3=length(Elev);
if (n1~=n2 | n1~=n3)
...
if (nargin>=5)
    ...
    %color
    if (strfind(style,'b')), color='b'; colorstring='7fff0000'; end
    if (strfind(style,'g')), color='g'; colorstring='7f00ff00'; end
    if (strfind(style,'r')), color='r'; colorstring='7f0000ff'; end
    if (strfind(style,'c')), color='c'; colorstring='7fffff00'; end
    if (strfind(style,'m')), color='m'; colorstring='7fff00ff'; end
    if (strfind(style,'y')), color='y'; colorstring='7f00ffff'; end
    if (strfind(style,'k')), color='k'; colorstring='7f000000'; end
    if (strfind(style,'w')), color='w'; colorstring='7fffffff'; end
end
